...
@pytest.mark.parametrize("raw", [
    ...
    '{"t":"hello","v":true,"token":"x"}', '{"t":"hello","v":4,"token":1}',
    '{"t":"hello","v":4,"token":"x","extra":0}',
    '{"t":"hello","v":4,"v":4,"token":"x"}',
    '{"t":"move","v":4,"direction":{"x":NaN,"y":0}}',
    '{"t":"move","v":4,"direction":{"x":2,"y":0}}',
    '{"t":"snapshot","v":4,"tick":0,"entities":[{}]}',
...
@pytest.mark.parametrize("message", [
    {"t": "hello", "v": 4, "token": ""},
    {"t": "move", "v": 4, "target": {"x": float("inf"), "y": 0}},
    {"t": "snapshot", "v": 4, "tick": 0, "entities": [], "extra": True},
    {"t": "delta", "v": 4, "tick": 0, "entered": [], "changed": [], "left": ["a", "a"]},
    {"t": "inventory", "v": 4, "tick": -1, "slots": []},
])
...
def test_entity_kinds(kind: str) -> None:
    ...
    message = {"t": "snapshot", "v": 4, "tick": 0, "entities": [entity]}
    assert decode(encode(message)) == message
...
def test_app_transport_and_cleanup(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        with client.websocket_connect("/ws") as socket:
            ...
            assert vitals == {"t": "vitals", "v": 4, "hp": 100, "hunger": 100}
            ...
            socket.send_json({"t": "move", "v": 4, "direction": {"x": 0, "y": 0}})
            socket.send_json({"t": "hello", "v": 4, "token": "anything"})
...
def test_schema_v4_rejects_v3() -> None:
    assert SCHEMA_VERSION == 4
    with pytest.raises(ProtocolError):
        decode('{"t":"hello","v":3,"token":"x"}')
...
def test_invalid_chunk_rle(runs: Any) -> None:
    message = {"t": "chunk", "v": 4, "cx": 0, "cy": 0, "size": 32, "tiles": runs}
    with pytest.raises(ProtocolError):
        encode(message)
    with pytest.raises(ProtocolError):
...
def test_invalid_chunk_fields(field: str, value: Any) -> None:
    message = {"t": "chunk", "v": 4, "cx": 0, "cy": 0, "size": 32, "tiles": [[1024, 0]]}
    message[field] = value
    with pytest.raises(ProtocolError):
        encode(message)
...
def test_chunk_real_terrain_round_trip() -> None:
    ...
    for cx, cy in [(0, 0), (2, 2), (3, 1), (5, 5)]:
        ...
        message = {"t": "chunk", "v": 4, "cx": cx, "cy": cy, "size": 32,
...
@pytest.mark.parametrize("field,value", [("world_size", 0), ("chunk_size", 16),
                                          ("seed", True)])
def test_invalid_welcome_v4(field: str, value: Any) -> None:
...
def test_invalid_vitals(field: str, value: Any) -> None:
    message = {"t": "vitals", "v": 4, "hp": 100, "hunger": 100}
    message[field] = value
    with pytest.raises(ProtocolError):
        encode(message)
    with pytest.raises(ProtocolError):
...
def test_vitals_bounds_and_closed_fields() -> None:
    assert decode(encode({"t": "vitals", "v": 4, "hp": 0, "hunger": 100}))["hp"] == 0
    with pytest.raises(ProtocolError):
        encode({"t": "vitals", "v": 4, "hp": 100})
    with pytest.raises(ProtocolError):
        encode({"t": "vitals", "v": 4, "hp": 100, "hunger": 100, "entity_id": "other"})
